...
def trans_pred(pred):
    ...
    new_pred = [(pred == 1) | (pred == 3), (pred == 1) | (pred == 3) | (pred == 2), pred == 3]
    new_pred = [i.long() for i in new_pred]
    return torch.stack(new_pred, dim=0)
...
def val_epoch(model, loader, epoch, acc_func, args, model_inferer=None, post_label=None, post_pred=None):
    ...
    with torch.no_grad():
        for idx, batch_data in enumerate(loader):
            ...
            dice_list_sub = []
            val_outputs = trans_pred(logits.argmax(1)[0])
            val_outputs = val_outputs.data.cpu().numpy().astype(np.uint8)
            val_labels = trans_pred(target[0, 0, :, :, :])
            val_labels = val_labels.cpu().numpy()
            from utils.utils import dice
            for i in range(3):
                num[i] += (np.sum(val_labels[i] > 0) > 0).astype(np.uint8)
                tumor_Dice = dice(val_outputs[i] > 0, val_labels[i] > 0)
                dice_list_sub.append(tumor_Dice)

            if all_dice is None:
                all_dice = (np.asarray(dice_list_sub)).copy()
            else:
                all_dice = all_dice + np.asarray(dice_list_sub)
            # print("Tumor Dice accumulate:", (all_dice / num))
