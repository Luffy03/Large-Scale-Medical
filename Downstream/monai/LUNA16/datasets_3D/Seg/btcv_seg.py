...
class SegmentationBTCVTrainSet(SegmentationBaseTrainset):
    """
    ...
    def __init__(self,
        ...
        super(SegmentationBTCVTrainSet, self).__init__(config, base_dir, flag)
        self.flag = flag
        self.config = config
        ...
        self.all_masks = list(
            map(lambda x: x.replace('volume', 'gt'), self.all_images))

        self.all_images = list(map(lambda x: os.path.join(self._base_dir + '/volume', x), self.all_images))
        self.all_masks = list(map(lambda x: os.path.join(self._base_dir + '/gt', x), self.all_masks))
    ...
    def __getitem__(self, index):
        ...
        return torch.from_numpy(image_array).float(), \
               torch.from_numpy(label_array).float(), image_index
...
class SegmentationBTCVTestset(SegmentationBaseTestset):
    """
    Test dataset for segmentation in the LiTS dataset (LCS).
    """
    def __init__(self, config, base_dir, flag='train'):
        super(SegmentationBTCVTestset, self).__init__(config, base_dir, flag)
        self.config = config
        self._base_dir = base_dir + '/' + flag
        self.all_images = []
