...
class Swin(nn.Module):
    def __init__(self, args):
        ...
        self.out = UnetOutBlock(spatial_dims=args.spatial_dims, in_channels=args.feature_size, out_channels=args.out_channels)
    ...
    def forward_logits(self, encs, hidden_states_out_3, frozen):
        enc0, enc1, enc2, enc3, dec4 = encs
        if frozen:
            enc0, enc1, enc2, enc3, dec4 = enc0.detach(), enc1.detach(), enc2.detach(), enc3.detach(),dec4.detach()

        dec3 = self.decoder5(dec4, hidden_states_out_3)
    ...
    def forward(self, x_in, frozen=True):
        b = x_in.size()[0]
        ...
        embeddings = self.forward_encs(encs)

        logits = self.forward_logits(encs, hidden_states_out[3], frozen)

        return embeddings.view(b, -1), logits
...
class VoCoHead(nn.Module):
    ...
    def forward(self, img, crops, labels, omni_inputs, omni_labels):
        ...
        embeddings, semi_outputs = self.backbone(inputs, frozen=True)
        semi_outputs = semi_outputs[:total_size]

        _, omni_outputs = self.backbone(omni_inputs, frozen=False)
        ...
        seg_loss = dice_loss(omni_outputs, omni_labels)
