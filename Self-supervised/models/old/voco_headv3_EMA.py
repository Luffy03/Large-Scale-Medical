...
class VoCoHead(nn.Module):
    ...
    def forward(self, img, crops, labels):
        ...
        for i in range(batch_size):
            ...
            x, bases = img[i * sw_size:(i + 1) * sw_size], crops[i * 16:(i + 1) * 16]
            x_bases = torch.cat([x, bases], dim=0)

            students = self.student(x_bases)
            x_stu, bases_stu = students[:sw_size], students[sw_size:]

            self._EMA_update_encoder_teacher()
            with torch.no_grad():
                teachers = self.teacher(x_bases.detach())
                x_tea, bases_tea = teachers[:sw_size], teachers[sw_size:]
