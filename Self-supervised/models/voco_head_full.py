...
class VoCoHead(nn.Module):
    def __init__(self, args):
        ...
        self.student = projection_head(in_dim=args.feature_size, hidden_dim=hidden_dim, out_dim=out_dim)
        self.teacher = projection_head(in_dim=args.feature_size, hidden_dim=hidden_dim, out_dim=out_dim)
    ...
    def forward(self, img, crops, labels):
        ...
        student = self.student(embeddings)
        ...
        for i in range(batch_size):
            ...
            # if i == 0:
            #     print('labels and logits:', label[0].data, logits1[0].data)

            intra_loss = ce_loss(label, logits1) + ce_loss(label, logits2)
            intra += intra_loss
    ...
    def inter_volume(self, x_stu, x_tea, inter_bases_stu, inter_bases_tea):
        pred1 = online_assign(x_stu, inter_bases_tea)
        pred2 = online_assign(x_tea, inter_bases_stu)
        # print('inter pred:', pred2)

        inter_loss = ce_loss(pred1.detach(), pred2) + ce_loss(pred2.detach(), pred1)
